...
int main(int argc, char** argv) {
  int mode = 0;  //默认“None”
  int blurKernelSize = 25;
  int pixelBlockSize = 20;

  // ---- 1. 参数设置（对齐标准实现的默认参数）----
  std::string modelPath =
      "assets/face_detection_yunet_2023mar.onnx";  // 模型路径
  //加载猫猫
  ...
  while (true) {
    ...
    if (mode == 1) {
      modeText = "Mode 1: Blur | Kernel = " + std::to_string(blurKernelSize);
    }
    if (mode == 2) {
      modeText = "Mode 2: Pixelate | Block = " + std::to_string(pixelBlockSize);
    }
    if (mode == 3) modeText = "Mode 3: Cat Mask";

    cv::putText(frame, modeText, cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX,
    ...
    if (!faces.empty()) {
      for (int i = 0; i < faces.rows; ++i) {
        ...
        switch (mode) {
          case 1:
            filter.blurFace(frame, faceRect, blurKernelSize);
            break;

          case 2:
            filter.pixelateFace(frame, faceRect, pixelBlockSize);
            break;
          ...
          case 3: {
            ...
            cv::Rect bigFace(cx - newW / 2, cy - newH / 2, newW, newH);
        ...
        cv::putText(frame, cv::format("Conf: %.2f", conf),
                    cv::Point(x1, y1 - 10), cv::FONT_HERSHEY_SIMPLEX, 0.6,
                    cv::Scalar(0, 255, 0), 2);
    ...
    if (key == '[') {
      ...
      if (mode == 2) {
        pixelBlockSize -= 2;
        if (pixelBlockSize < 4) pixelBlockSize = 4;
      }
    }
    if (key == ']') {
      if (mode == 1) {
